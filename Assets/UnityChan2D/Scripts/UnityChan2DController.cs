...
public class UnityChan2DController : MonoBehaviour
{
    ...
    void Awake() // Start()보다 먼저 호출이 된다.
                 // 스크립트가 실행될 때 한 번만 호출된다.
                 // 스크립트가 비활성화 상태에서도 실행이 된다. 
                 // 코루틴 사용이 안됨.
    {
    ...
    void Move(float move, bool jump)
    {
        ...
        if (jump && m_isGround) //1단 점프
        {
            JumpAction();
            mCanDoubleJump = true;
        }
        else if (jump && mCanDoubleJump) //2단 점프
        {
            JumpAction();
    ...
    void FixedUpdate() // 주로 물리 엔진을 사용하는 경우에 일정 시간 간격으로 힘을 가할 때 사용
                       // 발생하는 주기가 일정하다.
    {
        ...
        // 두 꼭지점인 pointA와 pointB가 서로 대각선을 이루는 사각형 안에 layerMask형 변수에 지정된 
        // 레이어에 놓여진 콜라이더가 있는지를 판정함
        m_animator.SetBool("isGround", m_isGround);
    }

    //OnTriggerEnter : 두 물체가 충돌되었을 때..
    //OnTriggerStay : 두 물체가 지속적으로 충돌되고 있을 때..
    //OnTriggerExit : 두 물체가 충돌됐다가 떨어질 때..
